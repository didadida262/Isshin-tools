...
export function RcCarTrainingTool() {
      ...
      <header className="relative shrink-0 overflow-hidden border-b border-border-subtle px-5 py-4 md:px-6">
        ...
        <div className="relative max-w-2xl pr-24 md:pr-36">
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-muted">
            按阶段做，没过关不要跳。毕业标准：无线可控，并能讲清「没线为什么还会动」。
          </p>
        </div>
      </header>
...
function CarSilhouette() {
  return (
    <svg
      aria-hidden
      viewBox="0 0 160 100"
      className="pointer-events-none absolute -right-1 top-1/2 h-20 w-32 -translate-y-1/2 text-border md:right-5 md:h-24 md:w-40"
    >
      <path
        d="M28 58 L40 38 H110 L132 58 H140 V70 H128 C128 78 118 78 118 70 H52 C52 78 42 78 42 70 H28 Z"
        fill="none"
        stroke="currentColor"
        strokeWidth="2.5"
        strokeLinejoin="round"
      />
      <circle cx="52" cy="70" r="10" fill="none" stroke="currentColor" strokeWidth="2.5" />
      <circle cx="118" cy="70" r="10" fill="none" stroke="currentColor" strokeWidth="2.5" />
      <circle cx="52" cy="70" r="3" fill="currentColor" opacity="0.45" />
      <circle cx="118" cy="70" r="3" fill="currentColor" opacity="0.45" />
      <rect
        x="52"
        y="42"
        width="48"
      ...
      />
      <g opacity="0.55">
        <line x1="18" y1="48" x2="8" y2="44" stroke="currentColor" strokeWidth="1.5">
          <animate attributeName="opacity" values="0.2;0.8;0.2" dur="1.6s" repeatCount="indefinite" />
        </line>
        <line x1="18" y1="54" x2="6" y2="54" stroke="currentColor" strokeWidth="1.5">
          <animate attributeName="opacity" values="0.8;0.2;0.8" dur="1.6s" repeatCount="indefinite" />
        </line>
        <line x1="18" y1="60" x2="8" y2="64" stroke="currentColor" strokeWidth="1.5">
          <animate attributeName="opacity" values="0.2;0.8;0.2" dur="1.6s" begin="0.4s" repeatCount="indefinite" />
        </line>
      </g>
    </svg>
  )
    ...
    case 'formula':
      return (
        <section className="rounded-2xl border border-border-subtle bg-surface/40 p-4">
          ...
          <pre className="mt-3 overflow-x-auto font-mono text-[13px] leading-relaxed whitespace-pre-wrap text-foreground">
            {block.expr}
          </pre>
    ...
    case 'diagram':
      return (
        <section className="rounded-2xl border border-border-subtle bg-surface/40 p-4 md:p-5">
          ...
          <div className="mt-4">
            {block.kind === 'chain' && <ControlChainDiagram />}
            {block.kind === 'diffdrive' && <DiffDriveDiagram />}
            {block.kind === 'compare' && <RadioCompareDiagram />}
          </div>
        </section>
      )
    case 'warn':
...
export default RcCarTrainingTool
